...
int fread_wrapper(uint8_t *buf, int bytes_to_read, FILE *f, int *add_len, uint32_t fsize)
{
    int bytes_read = 0;
    if(*add_len == 1) {
...
int main(int argc, char** argv)
{
    ...
    while(length && !feof(fi)) {
        ...
        bytes_read = fread_wrapper((uint8_t *)buffer, bytes_to_read, fi, &add_length, file_size);
        if(bytes_read) {
            if(seg_flag) {
